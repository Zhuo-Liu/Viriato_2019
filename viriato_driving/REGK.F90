...
# include "perfmacro.h"
  ...
  use diag,       only: proc0_reads_to_all, proc0_reads_to_all_nogs, flows, b_field, predatasave, &
       &                datasavetest, kdatasave3d, test_of_test, cuts, gm_spectrum, k_energy, injected, &
...
!        end if
        ...
        call zderivgk2(gk,dzgk2) !zderivative
...
!51         format(8g16.8)
           write(33,53) savetime, CFL_frac*dx/vxmax, CFL_frac*dy/vymax, &
                ...
                (1./rhos_de)*dz/sqrt(ngtot*1.0), &
                dz*(1+max(nky,nkx_par*Npe)*de**2)**0.5/(max(nky,nkx_par*Npe)*rhos),&
                dz/(rhos_de*sqrt(ngtot + 1.0))),dti, &
                RELATIVE_ERROR, abs(akpar(j1,2,1)), &
                dz**2/(zdiffcoeff), &
                1/(zdiffcoeff*kperpmax**2*(max(bxmax,bymax))**2)!p_count*1.0   !p_count*1.0 
